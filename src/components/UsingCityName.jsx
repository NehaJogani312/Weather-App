   ...
   function getWeatherUsingCityName(name){
    const options = {
      ...
      axios.request(options).then(function (response) {
        ...
        setPlusTwoDay(response.data.forecast.forecastday[1].date)
    ...
    <>
        ...
        <div className={styles.card}>
          <div className={styles.leftBox}>
              ...
              {temp && <div className={styles.temp}>{temp.toFixed(0)}&deg;</div>}
          ...
          <div className={styles.rightBox}>
            <div className={styles.row}>
              <div className={styles.item1}>
                ...
                <table style={{width: '94%', margin: '10px auto 0', background: 'transparent'}}>
                    <tr style={{lineHeight: '30px', background: 'transparent'}}>
                      ...
                      {zeroDayMin && <td style={{fontWeight: '500', textAlign: 'center', background: 'transparent'}}>{zeroDayMin}</td>}
                      {plusOneDayMin && <td style={{fontWeight: '500', textAlign: 'center', background: 'transparent'}}>{plusOneDayMin}</td>}
                      {plusTwoDayMin && <td style={{fontWeight: '500', textAlign: 'center', background: 'transparent'}}>{plusTwoDayMin}</td>}
                    </tr>
                    <tr style={{lineHeight: '30px', background: 'transparent'}}>
                      <td style={{borderRight: '1px solid white', width: '20%', fontSize: '15px', fontWeight: '500', paddingLeft: '3px', background: 'transparent'}}>Max</td>
                      {zeroDayMax && <td style={{fontWeight: '500', textAlign: 'center', background: 'transparent'}}>{zeroDayMax}</td>}
                      {plusOneDayMax && <td style={{fontWeight: '500', textAlign: 'center', background: 'transparent'}}>{plusOneDayMax}</td>}
                      {plusTwoDayMax && <td style={{fontWeight: '500', textAlign: 'center', background: 'transparent'}}>{plusTwoDayMax}</td>}
                    </tr>
              ...
              <div className={styles.item2}>
                ...
                <div className={styles.footnote}>The dew point is {dewPoint.toFixed(0)}&deg; right now.</div>
              </div>
            </div>
            ...
            <div className={styles.row}>
              <div className={styles.item2}>
                ...
                {feelsLike && <div className={styles.details}>{feelsLike.toFixed(0)}&deg;</div>}
